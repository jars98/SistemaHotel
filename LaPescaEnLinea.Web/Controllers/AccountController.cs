        ...
        public async Task<IActionResult> RecuperarPass(Usuario model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    string strMensaje = "";
                    string urlAddress = UrlHelper.GetPath(Request) + "Account/Index/?usuario=" + model.Email + "&pass=" + Tools.SecurityManager.Decrypt(objUsr.Password);
                    ...
                    await _emailSender.SendEmailAsync(model.Email, "Recuperación de cuenta de La Pesca en Línea ","Hola mundo");
